...
hook.Add( "HUDShouldDraw", "HideNonSimpleHUD", function( name )
	if Simple:GetBool() then
		if name == "CHudHealth" or name == "CHudBattery" then
			return false
		elseif ShowAmmo:GetBool() and ( name == "CHudAmmo" or name == "CHudSecondaryAmmo" ) then
			return false
		end
	end
end )
...
hook.Add( "PopulateToolMenu", "SimpleHUDSettings", function()
	spawnmenu.AddToolMenuOption( "Options", "HUD", "SimpleHUD_Options", "Simple HUD", "", "", function( panel )
		...
		refreshbutton.DoClick = RefreshStuff
		...
		resettodefault.DoClick = function()
			Derma_Query( "Are you sure?", "Simple HUD", "Yes", function()
				...
				RunConsoleCommand( "SimpleHUD_SpeedVehicleColor", "75 186 56 255" )
				timer.Simple( 0.3, RefreshStuff )
			end, "No", function() end )
		end
		...
		resettodefaulticons.DoClick = function()
			Derma_Query( "Are you sure?", "Simple HUD", "Yes", function()
				...
				SpeedInCarMat = Material( "icon16/car.png", "noclamp smooth" )
				timer.Simple( 0.3, RefreshStuff )
			end, "No", function() end )
		end	
